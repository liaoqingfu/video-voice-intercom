...
namespace XmlRpc {
  ...
  typedef  void (Executor) (XmlRpcValue& params, XmlRpcValue& result);
  
  //! Abstract class representing a single RPC method
  class XmlRpcServerMethod {
  public:
    //! Constructor
    XmlRpcServerMethod(std::string const& name, XmlRpcServer* server, bool needAuth = true);
    XmlRpcServerMethod(XmlRpcServer* server, std::string const& name, Executor* executor, bool needAuth = true);

    bool needAuth() { return _needAuth; }
    //! Destructor
