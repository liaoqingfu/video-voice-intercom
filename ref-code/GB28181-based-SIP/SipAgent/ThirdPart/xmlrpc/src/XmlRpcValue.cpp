...
#if defined(_WINDOWS)
#define INT64_OVERFLOW 9223372036854775807LL
...
strtoll (const char *nptr, char **endptr, int base)
{
  ...
  /* Parse positive number, checking for overflow. */
    int val;
    for (; (val = char_val(*nptr, base)) != -1; ++nptr)
    {
      long long newresult = base * result + val;
      if (newresult < result)
      {
...
namespace XmlRpc {
  ...
  static const char LONG_TAG[]       = "<long>";
  static const char LONG_ETAG[]       = "</long>";
  ...
  // Type checking
  std::string XmlRpcValue::getStringType(Type t)
  {
    std::string strType;
    switch (t) {
        case TypeBoolean:   strType = "Boolean";    break;
        case TypeInt:       strType = "Int";        break;
        case TypeLong:      strType = "Long";       break;
        case TypeDouble:    strType = "Double";     break;
        case TypeString:    strType = "String";     break;
        case TypeDateTime:  strType = "DateTime";   break;
        case TypeBase64:    strType = "Base64";     break;
        case TypeArray:     strType = "Array";      break;
        case TypeStruct:    strType = "Struct";     break;
        default:            strType = "Invalid";    break;
    }
    return strType;
  }
  ...
  void XmlRpcValue::assertTypeOrInvalid(Type t)
  {
    ...
    {
      std::string message = "type error";
      message += ", expected type: ";
      message += getStringType(t);
      message += ", but type: ";
      message += getStringType(_type);
      throw XmlRpcException(message);
    }
  }
  ...
  void XmlRpcValue::assertStruct(const std::string& s)
  {
    // yulichun Add:
    // 当该对象为string类型(通过服务器返回时空对象则为string),且size()为0,清空该对象
    if(_type == TypeString && size() == 0)
      invalidate();
    // End Add.
    ...
    {
    {
      std::string message = "type error: expected a struct";
      message += ", [\"";
      message += s;
      message += "\"]";
      throw XmlRpcException(message);
  ...
  XmlRpcValue& XmlRpcValue::operator=(XmlRpcValue const& rhs)
  {
    if (this != &rhs)
    {
      ...
      switch (_type) {
        ...
        case TypeLong:     _value.asLong = rhs._value.asLong; break;
  ...
  bool XmlRpcValue::operator==(XmlRpcValue const& other) const
  {
    ...
    switch (_type) {
      ...
      case TypeLong:     return _value.asLong == other._value.asLong;
  ...
  int XmlRpcValue::size() const
  {
    ...
    throw XmlRpcException("can not get size of TypeInvalid");
  }
  ...
  void XmlRpcValue::erase(int i)
  {
    if( _type == TypeArray )
      _value.asArray->erase(_value.asArray->begin() + i );
  }
  // Erase a struct member
  void XmlRpcValue::erase(const std::string& name)
  {
    if( _type == TypeStruct && _value.asStruct->find(name) != _value.asStruct->end()){
      _value.asStruct->erase(name);
    }
  }
  ...
  bool XmlRpcValue::fromXml(std::string const& valueXml, int* offset)
  {
    ...
    else if (typeTag == I8_TAG || typeTag == LONG_TAG)
  ...
  // Long
  bool XmlRpcValue::longFromXml(std::string const& valueXml, int* offset)
  {
    const char* valueStart = valueXml.c_str() + *offset;
    char* valueEnd;
    long long ivalue = strtoll(valueStart, &valueEnd, 10);
    if (valueEnd == valueStart)
      return false;

    _type = TypeLong;
    _value.asLong = (long long)(ivalue);
    *offset += int(valueEnd - valueStart);
    return true;
  }
  ...
  {
    char buf[256];
    snprintf(buf, sizeof(buf)-1, "%lld", _value.asLong);
    buf[sizeof(buf)-1] = 0;
    std::string xml = VALUE_TAG;
    xml += LONG_TAG;
    xml += buf;
  ...
  std::ostream& XmlRpcValue::write(std::ostream& os) const {
    switch (_type) {
      ...
      case TypeLong:     os << _value.asLong; break;
      case TypeDouble:   ///>>>>>BSTAR
        {
          char buf[256];
          snprintf(buf, sizeof(buf)-1, getDoubleFormat().c_str(), _value.asDouble); 
          buf[sizeof(buf)-1] = 0;		
...
//Convert any value to a string
  std::string XmlRpcValue::toString() const { 
