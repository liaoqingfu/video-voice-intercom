...
namespace XmlRpc {
  ...
  public:
    ...
    //! @param timeout The seconds before return
    ...
    bool isConnected() const { return _connectionState >CONNECTING; }

    // 设置连接使用的seesion号,和用户参数
    void setUserCookie(int sessionId, int userSpec);
  ...
  protected:
    ...
    virtual std::string generateSequence();
    virtual bool generateRequest(const char* method, XmlRpcValue const& params);
    virtual std::string generateHeader(const char* method, std::string const& body);
    ...
    // Number of seconds the client to wait the connection to the server
    int _waitTimeout;
    ...
    // Magic string in request header's referer, used for verification
    std::string _refererClient;
    // Magic string in response header' referer, copied from the request by the server
    std::string _refererServer;
    ...
    // Number of continuous connection failure
    int _retries;

    // 客户端可指定该连接的session号和一个整形值
    int _sessionId;
    int _userSpec;
