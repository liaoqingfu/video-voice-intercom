...
namespace XmlRpc {
  ...
  public:
    ...
    enum EventType {
      ...
      Exception     = 4,    //!< uh oh
      // PendingEvent  = 8     //!< pending on thread
    };
    ...
    //!  epoll and select separately
#if !defined(_WINDOWS)
    void workByEpoll(double msTime);
#endif
    void workBySelect(double msTime);
  ...
  private:
    ...
    struct MonitoredSource {
      MonitoredSource(XmlRpcSource* src, unsigned mask) : _src(src), _mask(mask), _erasable(false) {}
      XmlRpcSource* getSource() const { return _src; }
      unsigned getMask() { return _mask; }
      void setMask(unsigned mask) { _mask = mask; }
      bool getErasable() const { return _erasable; }
      void setErasable(bool erasable) { _erasable = erasable; }
      XmlRpcSource* _src;
      unsigned _mask;
      bool _erasable;
    };
    static bool canErase( const MonitoredSource&  source); 
