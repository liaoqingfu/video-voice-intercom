...
void testLong()
{
  XmlRpcValue long0((long long)0);
  XmlRpcValue long1((long long)1);
  XmlRpcValue long10((long long)10);
  XmlRpcValue long_1((long long)-1);
  int offset = 0;
  XmlRpcValue long0Xml("<value><long>0</long></value>", &offset);
  offset = 0;
  XmlRpcValue long9Xml("<value><i8>9</i8></value>", &offset);

  assert(long0 == long0Xml);
  assert((long long)(long10) - (long long)(long1) == (long long)(long9Xml));
  assert(9 == (long long)(long9Xml));
  assert((long long)(long10) + (long long)(long_1) == (long long)(long9Xml));

  long long x = -12345678901234567LL;
  XmlRpcValue longx(x);
  offset = 0;
  XmlRpcValue longxXml("<value><long>-12345678901234567</long></value>", &offset);
  long long y = longx;
  assert( x == y);
  assert( x == (long long)longxXml);

  XmlRpcValue params;
  params["long"] = y;
  assert( y == (long long)params["long"]);
...
void testDateTime()
{
  // DateTime
  int offset = 0;
  XmlRpcValue dateTime("<value><dateTime.iso8601>19870203T03:12:35</dateTime.iso8601></value>", &offset);
  struct tm &t = dateTime;
  assert(t.tm_year == 87 && t.tm_min == 12);

  std::cout << asctime( &t ) << std::endl;
  time_t now = tm_time(t);
  std::cout << now << std::endl;
  now += 1;
  
  struct tm tmp = time_tm(now);
  std::cout << asctime( &tmp ) << std::endl;
